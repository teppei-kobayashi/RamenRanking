...
/**
 * Servlet implementation class LoginUsersUpdateServlet
 */
@WebServlet("/loginusers/update")
public class LoginUsersUpdateServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public LoginUsersUpdateServlet() {
        super();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(_token != null && _token.equals(request.getSession().getId())) {
            ...
            // 重複チェックを行う指定をする
            Boolean mail_address_duplicate_check = true;
            if(l.getMail_address().equals(request.getParameter("mail_address"))) {
                mail_address_duplicate_check = false;
            } else {
            ...
            List<String> errors = LoginUserValidator.validate(l, mail_address_duplicate_check, password_check_flag);
            if(errors.size() > 0) {
                em.close();
                ...
                request.getSession().removeAttribute("loginuser_id");
